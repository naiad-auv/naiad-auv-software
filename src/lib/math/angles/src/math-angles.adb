...
package body Math.Angles is
   ...
   begin
      fNewAngle := fAngle;
      while fNewAngle <= fRangeFromExclusive loop
         fNewAngle := fNewAngle + (2.0 * Ada.Numerics.Pi);
      end loop;
      while fNewAngle > (fRangeFromExclusive + (2.0 * Ada.Numerics.Pi)) loop
         fNewAngle := fNewAngle - (2.0 * Ada.Numerics.Pi);
      end loop;
      return fNewAngle;
