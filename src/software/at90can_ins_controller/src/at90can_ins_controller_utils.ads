...
package AT90CAN_Ins_Controller_Utils is
   ...
   procedure Init_Uart(port : AVR.AT90CAN128.USART.USARTID; baud_rate : AVR.AT90CAN128.USART.BAUDTYPE);
   procedure Init_Interrupts;
   procedure Communication_Protocol_Control(port : AVR.AT90CAN128.USART.USARTID);
   procedure Async_Data_Output_Type_Register_Off(port : AVR.AT90CAN128.USART.USARTID);
   ...
   procedure Serial_Baud_Rate_Register(port : AVR.AT90CAN128.USART.USARTID);

   procedure Start_Message(sMsgStr : String; port : AVR.AT90CAN128.USART.USARTID);
