...
type TArrayOfPidScalings is array (X .. DriftZ) of simulator.Model.TPIDComponentScalings;
...
--  end read only
      ...
      pxModel : simulator.Model.pCModel := simulator.Model.pxCreate;
   ...
   begin
      ...
      for i in simulator.Model.EMotionComponent(X) .. simulator.Model.EMotionComponent(DriftZ) loop
...
--  end read only
   ...
   begin
      pxModel.pxMotionControlWrapper.Update_Wanted_Position(xWantedPosition    => xPositionVector,
                                                            xWantedOrientation => xOrientationMatrix);
      pxModel.fGet_Motor_Force(iIndexMotor => 1);
      aunit.Assertions.Assert(Condition => pxModel.xGet_Wanted_Submarine_Orientation_Matrix = xOrientationMatrix,
                              Message   => "Simulator.Model.xGetWanted_Submarine_Orientation_Matrix failed, Faulty Value");
...
--  end read only
   ...
   begin
      ...
      pxModel.pxSubmarine.Set_Position_Vector(xPositionVector => xPositionVector);
 --     aunit.Assertions.Assert(Condition => pxModel.pxMotionControlWrapper.Update_Values = xPositionVector,
  --                            Message   => "Simulator.Model.Set_Pid_Scaling failed, Faulty Value");
--  begin read only
