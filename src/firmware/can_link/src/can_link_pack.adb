...
package body CAN_Link_pack is
   ...
   begin
      ...
      if AVR.AT90CAN128.USART.iNumBytesLeft < CAN_Link_Utils.HEADLEN  then
         return;
      end if;

      Usart_Read(Head_Buf, USART_PORT, CAN_Link_Utils.HEADLEN, iRnum);

      CAN_Link_Utils.Bytes_To_Message_Header(Head_Buf, msg);

      if Integer(msg.Len) /= 0 then
         while AVR.AT90CAN128.USART.iNumBytesLeft < Integer(msg.Len) loop
            null;
         end loop;
         ...
         begin
            ...
            CAN_Link_Utils.Bytes_To_Message_Data(Data_Buf, msg, bChecksum);
         end;

         --At present, we just ignore this message if the checksum is wrong.
         if not bChecksum then
            return;
         else
            AVR.AT90CAN128.CAN.Can_Send(msg);
         end if;
